...
public class Condottiere extends Personnage {
    ...
    // Constructeur de la classe Condottiere
    public Condottiere() {
    ...
    public void utiliserPouvoir() {
        ...
        // Vérifier si le personnage est assassiné
        if (this.getAssassine()) {
            // Afficher un message en cas d'assassinat
            System.out.println("Votre personnage a été assassiné.");
        } else {
            // Créer une instance d'Interaction pour lire la réponse du joueur
            ...
            if (reponse) {
                // Afficher la liste des joueurs et le contenu de leur cité
                    ...
                    for (int j = 0; j < this.getPlateau().getJoueur(i).nbQuartiersReelDansCite(); j++) {
                        if (this.getPlateau().getJoueur(i).getCite()[j] != null) {
                            System.out.println("    " + (j + 1) + "." + this.getPlateau().getJoueur(i).getCite()[j].getNom() +
                                    " (" + (this.getPlateau().getJoueur(i).getCite()[j].getCout() - 1) + " pièces" + "),");
